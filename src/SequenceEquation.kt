...
For each x where 1 ≤ x ≤ n, that is x increments from 1 to n, find any integer y such that p(p(y)) = x and keep a history of the values of y in a return
array.
Example
p = [5, 2, 1, 3, 4]
Each value of x between 1 and 5, the length of the sequence, is analyzed as follows:
1. x = 1 =p[3], p[4] = 3, so p[p4]] = 1
...
fun permutationEquation(p: Array<Int>): Array<Int> {
    ...
    for (x in 1..n) {
        val indexX = p.indexOf(x) + 1

        val indexResult = p.indexOf(indexX) + 1

        result[x - 1] = indexResult
    }
